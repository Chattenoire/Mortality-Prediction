...
##############################################################################
# 3. BUILD CNN MODEL WITH HYPERPARAMETER TUNING
##############################################################################
def build_cnn_model(hp, input_shape):
    ...
    x = tf.keras.layers.Conv1D(filters=conv1_filters, kernel_size=kernel_size, activation="relu")(inputs)
    x = tf.keras.layers.MaxPooling1D(pool_size=2)(x)
    x = tf.keras.layers.Conv1D(filters=conv2_filters, kernel_size=kernel_size, activation="relu")(x)
    x = tf.keras.layers.MaxPooling1D(pool_size=2)(x)
...
def tune_model(train_ds, val_ds, input_shape):
    tuner = kt.Hyperband(
        ...
        factor=3,
        directory="kt_dir",
        project_name="centralized_cnn_tuning"
    )
...
##############################################################################
# 5. MAIN FUNCTION: TRAINING, EVALUATION, & SHAP EXPLANATION
##############################################################################
def main():
    ...
    input_shape = (X_train.shape[1], feature_dim)  # (time_steps, feature_dim)
    
    # C) Hyperparameter tuning
    best_hps = tune_model(train_ds, val_ds, input_shape)
    
    # D) Train the best model
    best_model = build_cnn_model(best_hps, input_shape)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('AUC')
    plt.title("Centralized CNN Training History")
    plt.legend()
    plt.savefig("training_history_cnn.png")
    plt.close()
    print("Training history plot saved as 'training_history_cnn.png'")
    ...
    plt.title("ROC Curve - Centralized CNN")
    ...
    # F) SHAP EXPLANATION
    # Pass the raw 3D validation data to the explainer, then average over time for visualization.
    ...
    # Average over time (axis=1) and squeeze the last dimension.
    shap_values_avg = np.mean(shap_values, axis=1).squeeze(-1)  # shape: (samples, feature_dim)
    X_explain_avg = np.mean(X_explain, axis=1)  # shape: (samples, feature_dim)
    ...
    plt.savefig("shap_summary_plot_cnn.png")
    plt.close()
    print("SHAP summary plot saved as 'shap_summary_plot_cnn.png'")
    
    # G) Save the final model
    model_save_path = r"C:\Graduation Project\saved models\centralized_cnn"
